...
#         super(RoIPooling, self).__init__()
    ...
    def forward(self, image, image_ids, rois):
        [batch_size, num_rois] = rois.get_shape.as_list()[:2]
        image_ids = tf.expand_dims(image_ids, dim=1)
        image_ids = tf.tile(image_ids, [1, num_rois])
        image_ids = tf.reshape(image_ids, [-1, 1])
#         image_ids = torch.unsqueeze(image_ids, dim=1).expand(batch_size, num_rois).contiguous().view(-1, 1)
        rois = tf.reshape(rois, [-1, 4])
        rois = tf.concat([image_ids, rois], axis=1)
